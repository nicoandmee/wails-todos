      ...
      <section class="main" v-show="todos.length">
        <ul class="todo-list">
          <li class="todo" v-for="todo in todos" :key="todo.id" :class="{completed: todo.completed, editing: todo == editedTodo}">
            ...
            <input v-model="todo.title" v-model.lazy="todo.title" @keyup.esc="cancelEdit(todo)" @blur="doneEdit(todo)" v-todo-focus="todo == editedTodo" @keyup.enter="doneEdit(todo)" type="text" class="edit">
          </li>
        </ul>
      </section>
    </section>
...
export default {
  ...
  mounted() {
   Wails.Events.On("filemodified", () => {
      // file has been modified, so we reload list
      this.loadList();
    });
